...
"%notin%" <- Negate("%in%")
list_731re <- foreach(his_date = 31:length(url_731), combine = cbind, .verbose = T) %dopar% {#test
     ...
     ################if there is table 1 (pos) and 2 (death)
                        ...
                        table_pos_death$new_death[rownames(table_pos_death) %in% rownames(table_death)] <- 
                                table_death[rownames(table_death) %notin% 
                                                    c("Bronx","Brooklyn", "Manhattan", "Staten Island", "Queens", "Kings"),2]
...
table_731 <- list.cbind(list_731re)

# export press releases in May
# add the date row. press_release_0x is just for exporting
press_release_07 <- data.frame(matrix(data = NA, 
                                      nrow = nrow(table_731)+2, ncol = ncol(table_731)+1))
press_release_07[3:nrow(press_release_07), 2:ncol(press_release_07)] <- table_731
colnames(press_release_07) <- c("county", colnames(table_731))
press_release_07[,1] <- c("","county",rownames(table_731))
press_release_07[2, 2:ncol(press_release_07)] <- colnames(table_731)
press_release_07[1,seq(2,ncol(press_release_07), 3)] <- 
        as.character(as.Date(
                as.numeric(as.Date("2020-07-01"):as.Date("2020-07-31")),format = "%Y-%m-%d",
                origin = as.Date("2020-03-20")-18341))
#
write_xlsx(press_release_07, path = "./output/press_07.xlsx")

# remove unnecessary objects
rm(list_731re, table_731)
