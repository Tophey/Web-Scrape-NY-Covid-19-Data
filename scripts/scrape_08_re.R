...
"%notin%" <- Negate("%in%")
list_831re <- foreach(his_date = 1:31, combine = cbind, .verbose = T) %dopar% {#test
...
table_831 <- list.cbind(list_831re)

# export press releases in May
# add the date row. press_release_0x is just for exporting
press_release_08 <- data.frame(matrix(data = NA, 
                                      nrow = nrow(table_831)+2, ncol = ncol(table_831)+1))
press_release_08[3:nrow(press_release_08), 2:ncol(press_release_08)] <- table_831
colnames(press_release_08) <- c("county", colnames(table_831))
press_release_08[,1] <- c("","county",rownames(table_831))
press_release_08[2, 2:ncol(press_release_08)] <- colnames(table_831)
press_release_08[1,seq(2,ncol(press_release_08), 3)] <- 
        as.character(as.Date(
                as.numeric(as.Date("2020-08-01"):as.Date("2020-08-31")),format = "%Y-%m-%d",
                origin = as.Date("2020-03-20")-18341))
#

write_xlsx(press_release_08, path = "./output/press_08.xlsx")

# remove unnecessary objects
rm(list_831re, table_831)
