...
"%notin%" <- Negate("%in%")
list_930re <- foreach(his_date = 32:length(url_930), combine = cbind, .verbose = T) %dopar% {#test
...
# add 928- 930
list_92830 <- foreach(his_date = 59:61, combine = cbind, .verbose = T) %dopar% {#test
        urls<- url_930[his_date] 
        htmls <- read_html(urls, encoding = "ISO-8859-1") 
        if (press_930$table[his_date] == "Yes") { ## Level 1 if there are table(s)
                table_pos<- rvest::html_nodes(x = htmls,
                                              xpath = '//table[last()-1]') %>% 
                        html_table(fill = TRUE) %>% .[1] %>% as.data.frame()   
                table_death <- rvest::html_nodes(x = htmls,
...
list_930re[28:30] <- list_92830[1:3]



table_930 <- list.cbind(list_930re)
# export press releases in May
# add the date row. press_release_0x is just for exporting
press_release_09 <- data.frame(matrix(data = NA, 
                                      nrow = nrow(table_930)+2, ncol = ncol(table_930)+1))
press_release_09[3:nrow(press_release_09), 2:ncol(press_release_09)] <- table_930
colnames(press_release_09) <- c("county", colnames(table_930))
press_release_09[,1] <- c("","county",rownames(table_930))
press_release_09[2, 2:ncol(press_release_09)] <- colnames(table_930)
press_release_09[1,seq(2,ncol(press_release_09), 3)] <- 
        as.character(as.Date(
                as.numeric(as.Date("2020-09-01"):as.Date("2020-09-30")),format = "%Y-%m-%d",
                origin = as.Date("2020-03-20")-18341))
#

write_xlsx(press_release_09, path = "./output/press_09.xlsx")

# remove unnecessary objects
rm(list_930re, list_92830, table_930)
