...
list_430re <- foreach(his_date = 32:length(url_430), combine = cbind, .verbose = T) %dopar% {#test
...
table_430 <- list.cbind(list_430re) # 04/01-04/30

# export press releases in April
# add the date row. press_release_0x is just for exporting
press_release_04 <- data.frame(matrix(data = NA, 
                                      nrow = nrow(table_430)+2, ncol = ncol(table_430)+1))
press_release_04[3:nrow(press_release_04), 2:ncol(press_release_04)] <- table_430
colnames(press_release_04) <- c("county", colnames(table_430))
press_release_04[,1] <- c("","county",rownames(table_430))
press_release_04[2, 2:ncol(press_release_04)] <- colnames(table_430)
press_release_04[1,seq(2,ncol(press_release_04), 3)] <- 
        as.character(as.Date(
                as.numeric(as.Date("2020-04-01"):as.Date("2020-04-30")),format = "%Y-%m-%d",
                origin = as.Date("2020-03-20")-18341))
#

write_xlsx(press_release_04, path = "./output/press_04.xlsx")

# remove unnecessary objects
rm(list_430re, table_430)
