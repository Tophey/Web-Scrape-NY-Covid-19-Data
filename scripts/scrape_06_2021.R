...
list_0630_21 <- foreach(his_date = 1:length(url_0630_21), combine = cbind, .verbose = T) %dopar% {#test
        urls<- url_0630_21[his_date] 
        if (!is.na(urls) & press_0630_21$table[his_date] == "Yes") { ## Level 1 if there are table(s)
...
table_0630_21 <- list.cbind(list_0630_21)

# export press releases 
# add the date row. press_release_ is just for exporting
press_release_06_2021 <- data.frame(matrix(data = NA, 
                                           nrow = nrow(table_0630_21)+2, ncol = ncol(table_0630_21)+1))
press_release_06_2021[3:nrow(press_release_06_2021), 2:ncol(press_release_06_2021)] <- table_0630_21
colnames(press_release_06_2021) <- c("county", colnames(table_0630_21))
press_release_06_2021[,1] <- c("","county",rownames(table_0630_21))
press_release_06_2021[2, 2:ncol(press_release_06_2021)] <- colnames(table_0630_21)
press_release_06_2021[1,seq(2,ncol(press_release_06_2021), 3)] <- 
        as.character(as.Date(
                as.numeric(as.Date("2021-06-01"):as.Date(press_0630_21$date[nrow(press_0630_21)])),format = "%Y-%m-%d",
                origin = as.Date("2020-03-20")-18341))
#write the data for reference
write_xlsx(press_release_06_2021, path = "./output/press_06_2021.xlsx")

# remove unnecessary objects
rm(list_0630_21, table_0630_21)
